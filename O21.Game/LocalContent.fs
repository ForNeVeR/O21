...
type LocalContent = {
    ...
    static member Load(lifetime: Lifetime, window: WindowParameters): Task<LocalContent> = task {
        let binDir = nonNull <| Path.GetDirectoryName(Environment.ProcessPath)
        ...
        let loadFont path = task {
            ...
            return RaylibUtils.LoadFontFromMemory lifetime (nonNull <| Path.GetExtension path) data fontSize fontChars
        }

        let loadTexture path = task {
            let! data = File.ReadAllBytesAsync(path)
            return RaylibUtils.LoadTextureFromMemory lifetime (nonNull <| Path.GetExtension path) data
        }
