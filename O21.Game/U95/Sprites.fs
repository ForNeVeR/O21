...
module Sprites =
    ...
    let private isColor (struct (r1, g1, b1), struct (r2, g2, b2)) = r1 = r2 && g1 = g2 && b1 = b2
    ...
    let private loadBackgrounds (backgroundGraphics: Dib[]) : Texture[] =
        Array.init backgroundGraphics.Length (fun i -> createSprite backgroundGraphics[i] backgroundGraphics[i])
    ...
    let private createFish (index: int) (fishGraphics: Dib[]) : Fish =
...

        {
            ...
            LeftDirection = Array.init 8 (fun i -> createSprite fishGraphics[shift + i + 45] fishGraphics[shift + i])

            RightDirection =
                Array.init 8 (fun i -> createSprite fishGraphics[shift + i + 135] fishGraphics[shift + i + 90])
    ...
    let private loadFishes (fishGraphics: Dib[]) : Fish[] =
        Array.init (fishGraphics.Length / 36) (fun i -> createFish i fishGraphics)

    let private loadHUD (hudGraphics: Dib[]) =
        Array.init (hudGraphics.Length) (fun i -> createSprite hudGraphics[i] hudGraphics[i])
