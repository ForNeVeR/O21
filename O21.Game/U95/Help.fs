...
let private convertParagraphs (fonts: FontDescriptor[]) (bitmaps: int -> Texture) (items: IParagraphItem seq) = seq {
    ...
    for item in items do
        ...
        | :? ParagraphText as pc -> yield DocumentFragment.Text(style currentFont, pc.Text)
        | :? NewParagraph -> yield DocumentFragment.NewParagraph
        | :? Bitmap as b -> yield DocumentFragment.Image(bitmaps <| int b.Number)
...
let Load (helpFile: string) : Task<DocumentFragment[]> = task {
    ...
    let files =
        helpFile.GetFiles Encoding.UTF8 |> Seq.map (fun d -> d.FileName, d) |> Map.ofSeq

    let fonts = helpFile.ReadFile files["|FONT"] |> loadFontDescriptors
